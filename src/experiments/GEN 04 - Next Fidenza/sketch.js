...
function draw() {
  ...
  // SETUP
  let circleSteps = integrerRandom(80,200)
  let circleMin = 20
  let circleMax = canvasSizeH * 2
  let circleX = integrerRandom(250,750)
  let circleY = integrerRandom(250,750)
  ...
  for(let drawCircle = 0; drawCircle < circleSteps; drawCircle++){
    ...
    strokeWeight(integrerRandom(1,10))
    stroke(strokeColor)
    noFill()
    circle(circleX, circleY, integrerRandom(circleMin,circleMax))

    let arcSize = integrerRandom(circleMin,circleMax)
    let arcStart = integrerRandom(0,360)
    let arcStop = arcStart + integrerRandom(0, 360 - arcStart)
    strokeColor = colorPalette[integrerRandom(0,5)]
    drawingContext.setLineDash([integrerRandom(50,200), integrerRandom(0,50)])
    strokeWeight(integrerRandom(1,10))
    stroke(strokeColor)
    noFill()
    arc(circleX, circleY, arcSize, arcSize, arcStart, arcStop)
  }
}
...
//   noFill()
//   strokeCap(SQUARE)
//   for(let drawCircle = 0; drawCircle < 40; drawCircle++){
//     drawingContext.setLineDash([integrerRandom(50,200), integrerRandom(0,50)])
//     strokeWeight(integrerRandom(1,10))
//     let circleX = integrerRandom(0,canvasSizeH)
//     let circleY = integrerRandom(0,canvasSizeV)
//     stroke(palette[integrerRandom(0,palette.length - 1)])
//     circle(circleX, circleY, integrerRandom(circleMin,circleMax))
...
//     let arcStart = integrerRandom(0,360)
//     let arcStop = arcStart + integrerRandom(0, 360 - arcStart)
//     arc(arcX, arcY, arcSize, arcSize, arcStart, arcStop)
//   }
// }
